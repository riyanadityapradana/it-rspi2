		...
		<div class="card">
		    ...
		    <div class="card-body">
		    	<div class="row mb-3">
		    		<div class="col-md-3">
		    			...
		    			<select class="form-control" id="filterUser" name="user">
		    				...
		    				<?php
		    				// Ambil list user dengan status aktif
		    				$queryUser = mysqli_query($config, "SELECT id_user, nama_lengkap FROM tb_user WHERE status = 'aktif' ORDER BY nama_lengkap");
		        ...
		        <table id="example1" class="table table-bordered table-striped">
		          	...
		          	$whereClause = "WHERE MONTH(l.tanggal_log) = '$bulan' AND YEAR(l.tanggal_log) = '$tahun' AND u.status = 'aktif'";
		          	if ($user != '') {
		          		$whereClause .= " AND l.id_user = '$user'";
		          	}
		          	
		          	$query = mysqli_query($config,"SELECT * FROM tb_logbook l JOIN tb_user u ON l.id_user=u.id_user $whereClause ORDER BY l.tanggal_log DESC")or die(mysqli_error($config));
		          	$n=1;
